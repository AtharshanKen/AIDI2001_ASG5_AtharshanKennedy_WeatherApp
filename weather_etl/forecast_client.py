...
def normalize_daily_response(response, city_id: str) -> dict:
    ...
    return {
        "daily": {
            ...
            "temperature_2m_min": list(daily.Variables(0).ValuesAsNumpy()),
            "temperature_2m_max": list(daily.Variables(1).ValuesAsNumpy()),
            "precipitation_sum": list(daily.Variables(2).ValuesAsNumpy()),
            "wind_speed_10m_max": list(daily.Variables(3).ValuesAsNumpy()),
            "weather_code": list(daily.Variables(4).ValuesAsNumpy()),
        }
    }
